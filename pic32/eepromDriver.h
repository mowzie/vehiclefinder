...
// Maximum number of 32-bit data
	#define DATA_EE_SIZE       			 (680) // Default configuration.
//	#define DATA_EE_SIZE       			(1024) // Minimum 4 Flash pages required.

// Total number of pages reserved for the operation
	#define NUM_DATA_EE_PAGES   		   (1) // Minimum. No more than 680 values + updates in total.
//	#define NUM_DATA_EE_PAGES   		   (2) // Small configuration.
//	#define NUM_DATA_EE_PAGES			   (3) // Default configuration
//	#define NUM_DATA_EE_PAGES			   (4) // Minimum number of pages for DATA_EE_SIZE 1024.
//	#define NUM_DATA_EE_PAGES			   (5) // 

// Maximum erase cycles per page
	#define ERASE_WRITE_CYCLE_MAX          (5) // Use a small value for testing 
//	#define ERASE_WRITE_CYCLE_MAX		(1000) // Maximum erase cycle per page
    
// Fixed eedata_addr[][]  array address in Flash Program Memory
//	#define EEDATA_ADDRESS		  (0x9D04D000) // Must be aligned a multiple of 0x01000 on PIC32MX
